...
const float F3 = 0.366025404; // (sqrt(3) - 1) / 2;
const float G3 = 0.211324865; // (3 - sqrt(3)) / 6;
...
float simplex2d( in vec2 position )
{
    ...
    // calculate s and x
	vec2 s = floor(position + dot(position, vec2(F3))); //- generalizes to higher dimensions
                                                        //vec2 s = floor( position + (position.x + position.y) * F3 ); -- this is interchangeable.
    vec2 x1 = position - s + dot(s, vec2(G3));          //- generalizes to higher dimensions
